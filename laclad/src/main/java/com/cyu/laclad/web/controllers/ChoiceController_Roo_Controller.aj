...
import com.cyu.laclad.web.controllers.ChoiceController;
...
privileged aspect ChoiceController_Roo_Controller {
    ...
    public String ChoiceController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "sortFieldName", required = false) String sortFieldName, @RequestParam(value = "sortOrder", required = false) String sortOrder, Model uiModel) {
        if (page != null || size != null) {
            int sizeNo = size == null ? 10 : size.intValue();
            final int firstResult = page == null ? 0 : (page.intValue() - 1) * sizeNo;
            uiModel.addAttribute("choices", Choice.findChoiceEntries(firstResult, sizeNo, sortFieldName, sortOrder));
            float nrOfPages = (float) Choice.countChoices() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("choices", Choice.findAllChoices(sortFieldName, sortOrder));
        }
        return "choices/list";
    }
