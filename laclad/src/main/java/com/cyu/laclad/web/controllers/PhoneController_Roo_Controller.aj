...
import com.cyu.laclad.web.controllers.PhoneController;
...
privileged aspect PhoneController_Roo_Controller {
    ...
    public String PhoneController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "sortFieldName", required = false) String sortFieldName, @RequestParam(value = "sortOrder", required = false) String sortOrder, Model uiModel) {
        if (page != null || size != null) {
            int sizeNo = size == null ? 10 : size.intValue();
            final int firstResult = page == null ? 0 : (page.intValue() - 1) * sizeNo;
            uiModel.addAttribute("phones", Phone.findPhoneEntries(firstResult, sizeNo, sortFieldName, sortOrder));
            float nrOfPages = (float) Phone.countPhones() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("phones", Phone.findAllPhones(sortFieldName, sortOrder));
        }
        return "phones/list";
    }
    ...
    public String PhoneController.updateForm(@PathVariable("id") Long id, Model uiModel) {
        populateEditForm(uiModel, Phone.findPhone(id));
        return "phones/update";
    }
    ...
    public String PhoneController.delete(@PathVariable("id") Long id, @RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
        Phone phone = Phone.findPhone(id);
        phone.remove();
        uiModel.asMap().clear();
